...
def set_lease_details(db: Session, case_no, step_data):
    ...
    try:
        ...
        lease_data = {
            ...
            "total_segments": 8 if step_data.get("lease_type") == "dov" else None,
        }
...
def set_driver(db, case_no, step_data):
    ...
    try:
        ...
        if lease.lease_type == "dov":
            ...
            logger.info("\n")
        elif lease.lease_type == "long-term":
            logger.info("\n")
        ...
        elif lease.lease_type == "shift-lease":
            logger.info("\n")
        ...
        elif lease.lease_type == "medallion-only":
            logger.info("********** Generating dov medallion document **********")
            medallion_document_info = generate_medallion_lease_document(
                db, lease, authorized_agent
            )
...
async def send_document_for_signature(db, case_no, step_data):
    ...
    try:
        ...
        if lease.lease_type == "long-term" or lease.lease_type == "shift-lease":
        ...
        if lease.lease_type == "dov" or lease.lease_type == "medallion-only":
