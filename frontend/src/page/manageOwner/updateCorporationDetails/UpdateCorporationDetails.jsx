...
const UpdateCorporationDetails = ({
  caseId,
  currentStepId,
  isCaseSuccess,
  reload,
  ...
  console.log("Edit Corporation params", params);
  ...
  // --- MODIFICATION START: State for new features ---
  const [initialIsHoldingCompany, setInitialIsHoldingCompany] = useState(false);
  const [initialMailingAddress, setInitialMailingAddress] = useState(null);
  const [showRiderPrompt, setShowRiderPrompt] = useState(false);
  // --- MODIFICATION END ---
  ...
  const [isHoldingErrorActive, setIsHoldingErrorActive] = useState(false); // State to prevent toast stacking
  const didAddressChangeOnSubmit = useRef(false);
  const waitingForAddressRefresh = useRef(false);
  const waitingForDataRefresh = useRef(false);
  ...
  useEffect(() => {
    if (isProccessDataSuccess) {
      waitingForAddressRefresh.current = true;
      triggerGetStepInfo({ caseNo: caseId, step_no: currentStepId });
    }
  ...
  const parentHoldingCompanyOptions = useMemo(() => {
    ...
    if (corporationListData?.items) {
      const mappedCorps = corporationListData.items
        // Filter out the current corporation from the list to prevent self-selection.
        .filter((corp) => corp.id !== stepInfoData?.id)
        .map((corp) => ({
          name: corp.name,
          code: corp.id,
        }));
    ...
    return options;
  }, [corporationListData, stepInfoData]); // Add stepInfoData to dependencies
  ...
  const formik = useFormik({
    ...
    onSubmit: (values) => {
      console.log(
        "Validation passed. Generating document for values:",
        values,
        initialIsHoldingCompany,
        isHoldingCompany
      );

      // --- MODIFICATION START: Holding Company Validation ---
      // This function simulates a check to see if other companies are linked.
      // const hasAssociatedCompanies = (corporationId) => {
      //   console.log(
      //     `Checking for child associations for corporation ID: ${corporationId}`
      //   );
      //   return true;
      // };
      ...
      //   initialIsHoldingCompany &&
      //   !isHoldingCompany
      //   // stepInfoData?.no_of_child_entities >= 1
      // ) {
      //   toast.current.showToast(
      //     "Error",
      //     "Cannot remove Holding Company status as this company has one or more associated companies. Please disassociate or reassign them before changing its status",
      //     "error",
      //     false,
      //     10000
      //   );
      ...
      didAddressChangeOnSubmit.current = payload.is_update_address; // Capture submission intent
  ...
  useEffect(() => {
    if (isProccessDataSuccess) {
      if (didAddressChangeOnSubmit.current) {
        toast.current.showToast(
          "Success",
          "Rider Document and Updated Address Form Generated Successfully",
          "success",
          false,
          10000
        );
        ...
        toast.current.showToast(
          "Success",
          "Corporation details updated successfully.",
          "success",
          false,
          10000
        );
      }
      didAddressChangeOnSubmit.current = false; // Reset the flag
      triggerGetStepInfo({ caseNo: caseId, step_no: currentStepId }, true);
    }
  }, [isProccessDataSuccess, caseId, currentStepId, triggerGetStepInfo]);
  useEffect(() => {
    if (isProccessDataSuccess && !isFetching && stepInfoData) {
      ...
      setInitialMailingAddress({
        secondaryAdd: stepInfoData.is_mailing_address_same ?? true,
        ...
        secondaryAddress1: stepInfoData.secondary_address?.address_line_1 || "",
        secondaryAddress2: stepInfoData.secondary_address?.address_line_2 || "",
  ...
  const generatePayload = (values) => {
    const payload = {
      ...
      is_update_address: hasMailingAddressChanged(),
    };
    if (secondaryAdd) {
  ...
  useEffect(() => {
    ...
    if (stepInfoData && !isUpload) {
      ...
      // --- MODIFICATION START: Set initial holding company status ---
      const initialHoldingStatus = stepInfoData?.is_holding_entity ?? false;
      setIsHoldingCompany(initialHoldingStatus);
      setInitialIsHoldingCompany(initialHoldingStatus);
      // --- MODIFICATION END ---
      if (stepInfoData?.holding_entity) {
        ...
        formik.setFieldValue("parent_holding_company", {
          name: "None",
          code: "None",
        });
      }
      ...
      // Set the initial state baseline for comparison using the same exact data source
      ...
      setInitialMailingAddress({
        ...
        primaryAddress2: stepInfoData?.primary_address?.address_line_2 || "",
        primaryCity: stepInfoData?.primary_address?.city || "",
        primaryState: primaryStateObj || null, // Use consistent object
        primaryZip: stepInfoData?.primary_address?.zip || "",
        secondaryAddress1:
          stepInfoData?.secondary_address?.address_line_1 || "",
        secondaryAddress2:
          stepInfoData?.secondary_address?.address_line_2 || "",
        secondaryCity: stepInfoData?.secondary_address?.city || "",
        secondaryState: secondaryStateObj || null, // Use consistent object
        secondaryZip: stepInfoData?.secondary_address?.zip || "",
      });
  ...
  }, [stepInfoData, isUpload]); // This effect runs when the stepInfoData data changes
  ...
  const hasMailingAddressChanged = () => {
    if (!initialMailingAddress) return false;
    const { values } = formik;
    if (secondaryAdd !== initialMailingAddress.secondaryAdd) return true;
    if (
      values.primaryAddress1 !== initialMailingAddress.primaryAddress1 ||
      values.primaryAddress2 !== initialMailingAddress.primaryAddress2 ||
      values.primaryCity !== initialMailingAddress.primaryCity ||
      values.primaryState?.code !== initialMailingAddress.primaryState?.code ||
      values.primaryZip !== initialMailingAddress.primaryZip
    )
      return true;

    if (!secondaryAdd) {
      if (
        values.secondaryAddress1 !== initialMailingAddress.secondaryAddress1 ||
        values.secondaryAddress2 !== initialMailingAddress.secondaryAddress2 ||
        values.secondaryCity !== initialMailingAddress.secondaryCity ||
        values.secondaryState?.code !==
          initialMailingAddress.secondaryState?.code ||
        values.secondaryZip !== initialMailingAddress.secondaryZip
      )
        return true;
    }
    return false;
  ...
  const proceedToMoveCase = () => {
    if (hasAccess && caseData?.case_info.case_status !== "Closed") {
  ...
  const handleProcessFlowOnly = () => {
    if (hasAccess) {
      const payload = generatePayload(formik.values);
      didAddressChangeOnSubmit.current = payload.is_update_address; // Capture submission intent
      processFlow({
        params: caseId,
        data: {
        ...
        console.error("processFlow from modal failed:", error);
        toast.current.showToast(
          "Error",
  ...
  const completeAndMoveCase = async () => {
    // First, validate the entire form to ensure all fields are correct.
    const errors = await formik.validateForm();
    formik.setTouched(errors, true); // Mark fields as touched to display errors

    // If there are any validation errors, show a toast and stop.
    if (Object.keys(errors).length > 0) {
    ...
    //   toast.current.showToast(
    //     "Error",
    //     "Cannot remove Holding Company status as this company has one or more associated companies.",
    //     "error",
    //     false,
    //     10000
    //   );
    ...
    // If validation is successful, check if the mailing address has changed.
    if (hasMailingAddressChanged()) {
      // If the address changed, show the confirmation modal to the user.
      setShowRiderPrompt(true);
    } else {
      // If the address did not change, proceed directly with the API calls.
      handleFinalSubmit();
    }
  };
  ...
  useEffect(() => {
    if (waitingForDataRefresh.current && !isFetching) {
      waitingForDataRefresh.current = false;
      if (moveCaseOnSuccess.current) {
        proceedToMoveCase();
        moveCaseOnSuccess.current = false;
      }
  ...
  return (
    <div className="postion-relative">
      {/* <p className="sec-topic pb-3">Edit Corporation</p> */}
      <form
      ...
      >
          ...
          <div className="form-body">
            ...
            >
              <div className="w-100-3">
                {/* --- MODIFICATION START: Make field read-only --- */}
                <BInputText
              ...
              <div className="w-100-3">
                <div className="d-flex align-items-center gap-2">
                  <Checkbox
                    ...
                    // }}
                    onChange={(e) => {
                      const isTryingToUncheck = !e.checked;
                      ...
                      if (
                        ...
                        if (!isHoldingErrorActive) {
                          ...
                          setIsHoldingErrorActive(true);
                          setTimeout(() => {
                            setIsHoldingErrorActive(false);
                          }, 10000);
                        }
                        return;
                      }

                      setIsHoldingCompany(e.checked);
                      if (e.checked) {
                        formik.setFieldValue(
                          variable.parent_holding_company.id,
                          { name: "None", code: "None" }
        ...
        {/* The rest of the JSX remains the same until the final button */}
        {/* ... (All other form sections: Legal Address, Mailing Address, Beneficial Owners, Payees, Generate Document) ... */}
        ...
        <div className="w-100 position-sticky bottom-0 py-3 bg-white">
          <Button
            ...
            label="Update Corporation Details"
            type="button"
      ...
      {/* <BConfirmModal
        isOpen={isOpen}
        title="TLC requires Rider Document Update"
        message="Would you like to regenerate the Lease Rider now?"
        onCancel={() => {
          setOpen(false);
        }}
        onConfirm={() => {
      ...
      <BConfirmModal
        ...
        onCancel={() => {
          setShowRiderPrompt(false);
        }}
        onConfirm={() => {
          // When the user confirms, close the modal and proceed with the API calls.
          setShowRiderPrompt(false);
          handleProcessFlowOnly();
        }}
      ></BConfirmModal>
...
export default UpdateCorporationDetails;
