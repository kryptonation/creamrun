"""curb models implemented

Revision ID: 28a5fb4e46cd
Revises: c22bebe3dcdb
Create Date: 2025-10-28 10:49:10.049405
...
revision: str = '28a5fb4e46cd'
down_revision: Union[str, Sequence[str], None] = 'c22bebe3dcdb'
...
def upgrade() -> None:
    ...
    sa.Column('balance_id', sa.String(length=64), nullable=False, comment='Unique balance ID (Format: LB-YYYY-NNNNNN)'),
    ...
    sa.Column('reference_type', sa.String(length=128), nullable=False, comment='Type of obligation (e.g., EZPASS_TRANSACTION, LEASE_SCHEDULE)'),
    sa.Column('reference_id', sa.String(length=128), nullable=False, comment='Reference to source obligation record'),
    sa.Column('original_amount', sa.Numeric(precision=10, scale=2), nullable=False, comment='Original obligation amount'),
    sa.Column('prior_balance', sa.Numeric(precision=10, scale=2), nullable=False, comment='Balance brought forward from previous period'),
    sa.Column('current_amount', sa.Numeric(precision=10, scale=2), nullable=False, comment='New charges for current period'),
    sa.Column('payment_applied', sa.Numeric(precision=10, scale=2), nullable=False, comment='Total payments applied'),
    sa.Column('outstanding_balance', sa.Numeric(precision=10, scale=2), nullable=False, comment='Remaining unpaid balance'),
    sa.Column('payment_period_start', sa.DateTime(), nullable=False),
    sa.Column('payment_period_end', sa.DateTime(), nullable=False),
    sa.Column('due_date', sa.DateTime(), nullable=True, comment='When payment is due'),
    sa.Column('status', sa.Enum('OPEN', 'CLOSED', 'DISPUTED', 'VOIDED', name='balancestatus'), nullable=False),
    sa.Column('payment_reference', sa.Text(), nullable=True, comment='JSON array tracking payment application history'),
    ...
    sa.Column('created_on', sa.DateTime(timezone=True), server_default=sa.text('now()'), nullable=True, comment='Timestamp when this record was created'),
    sa.Column('updated_on', sa.DateTime(timezone=True), nullable=True, comment='Timestamp when this record was last updated'),
    sa.CheckConstraint('original_amount >= 0', name='check_original_amount'),
    sa.CheckConstraint('outstanding_balance >= 0', name='check_outstanding_balance'),
    sa.ForeignKeyConstraint(['created_by'], ['users.id'], onupdate='CASCADE', ondelete='SET NULL'),
    ...
    op.create_index('idx_balance_category_due_date', 'ledger_balances', ['category', 'due_date'], unique=False)
    op.create_index('idx_balance_driver_lease_status', 'ledger_balances', ['driver_id', 'lease_id', 'status'], unique=False)
    op.create_index('idx_balance_period', 'ledger_balances', ['payment_period_start', 'payment_period_end'], unique=False)
    ...
    op.create_index(op.f('ix_ledger_balances_payment_period_end'), 'ledger_balances', ['payment_period_end'], unique=False)
    op.create_index(op.f('ix_ledger_balances_payment_period_start'), 'ledger_balances', ['payment_period_start'], unique=False)
...
def downgrade() -> None:
    ...
    op.drop_index(op.f('ix_ledger_balances_payment_period_start'), table_name='ledger_balances')
    op.drop_index(op.f('ix_ledger_balances_payment_period_end'), table_name='ledger_balances')
    ...
    op.drop_index('idx_balance_period', table_name='ledger_balances')
    op.drop_index('idx_balance_driver_lease_status', table_name='ledger_balances')
    op.drop_index('idx_balance_category_due_date', table_name='ledger_balances')
