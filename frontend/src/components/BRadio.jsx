...
const BRadio = ({ variable, formik, isRequire, isDisable = false }) => {
  return (
    <div
    ...
    >
      <div className="d-flex align-items-center flex-wrap gap-3">
        <p className={`radio-main-label ${isDisable ? "disabled-color" : ""}`}>
          {variable.label}{" "}
        ...
        {variable.options ? (
          variable.options.map((option) => (
            <div className="d-flex align-items-center gap-2" key={option.value}>
              <RadioButton
                ...
                disabled={isDisable}
                className={`${isDisable ? "disabled-color" : ""}`}
              />
              <label htmlFor={`${variable.id}_${option.value}`} data-testid={`${variable.id}_${option.value}-lable`} className={`ml-2 radio-label ${isDisable ? "disabled-color" : ""}`}>
                {option.label}
              </label>
            </div>
          ))
          ...
          <>
            <p className="radio-main-label">{variable.label} {isRequire && <span className="require-star">*</span>}</p>
            <div className="d-flex align-items-center gap-2">
              <RadioButton
                ...
                isDisable={isDisable}
              />
              <label htmlFor={variable.options[0].id} data-testid={`${variable.options[0].id}-label`} className="ml-2 radio-label">
            ...
            <div className="d-flex align-items-center gap-2">
              ...
              />
              <label htmlFor={variable.options[1].id} data-testid={`${variable.options[1].id}-label`} className="ml-2">
