...
async def create_manual_audit_trail(
    ...
    try:
        ...
        audit_entry = audit_trail_service.create_audit_trail(
            db, case=case if case else None, description=entry.description, user=get_current_user, meta_data=meta_data, audit_type=AuditTrailType.MANUAL
        )
        return audit_entry
...
async def get_case_audit(
    ...
    try:
        ...
        for audit in audits:
            user_data = await user_service.repo.get_user_by_id(user_id=audit.created_by)
            results.append({
                "id":audit.id,
