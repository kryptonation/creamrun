...
@step(step_id="203" , name="Fetch - Vehicle Purchase Expenses", operation='fetch')
...
@step(step_id="203" , name="process -Edit Vehicle purchase expenses", operation="process")
def process_vehicle_purchase_expenses(db, case_no, step_data):
    """
    Process the edit vehicle purchase expenses and compliance data.
    """
    try:
        logger.info("Process edit vehicle purchase expenses for case_no: %s", case_no)
        ...
        if case:
            audit_trail_service.create_audit_trail(
                db=db,
                case=case,
                description=f"Edit Vehicle purchase expenses updated for vehicle vin:{expense.vehicle.vin if expense.vehicle else 'N/A'}",
                meta_data={"vehicle_id":expense.vehicle_id}
            )

        return "Ok"
    except Exception as e:
        logger.error("Error processing edit vehicle purchase expenses: %s", str(e))
        raise e