...
const UploadDocumentsStepComponent = ({
  caseId,
  currentStepId,
  reload,
  currentStep,
  ...
  const customRender = (column, rowData) => {
    if (column.field === "document_name") {
      ...
      const isRequired = stepInfoData?.required_documents?.includes(
  ...
  const areAllRequiredDocumentsUploaded = () => {
    ...
    if (!stepInfoData?.required_documents || !stepInfoData?.documents)
      return true;

    const requiredTypes = new Set(stepInfoData?.required_documents);
    const uploadedRequiredTypes = new Set();

    stepInfoData?.documents.forEach((doc) => {
      if (
...
export default UploadDocumentsStepComponent;
