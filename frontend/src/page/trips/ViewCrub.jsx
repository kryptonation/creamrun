...
const ViewCrub = () => {
  ...
  const [
    triggerGetEzpass,
    { data: ezpassDetail, isSuccess: isGridDataSuccess },
  ] = useLazyGetCurbQuery();
  const [triggerGetEzpassSearch, { data: ezpassSearchDetail }] =
    useLazyGetCurbQuery();

  const [filterSearchBy, setFilterSearchBy] = useState("");
  const [lazyAssociate] = useAssociateCurbBatmMutation();

  const tripId = "trip_id";
  const tripDate = "trip_start_date";
  const driverId = "driver_id";
  const cabNumber = "cab_number";
  const startLocationgps = "start_location_gps";
  const resolutionStatus = "log_status";
  const fields = [
    { key: "trip_id", label: "Trip ID", formatType: "search" },
    { key: "trip_start_date", label: "Trip Date", formatType: "date" },
    {
      key: "start_time",
    ...
    { key: "cab_number", label: "Cab Number", formatType: "Search" },
    {
      key: "start_location_gps",
      label: "Start Location GPS",
      filter: false,
      sortable: false,
    },
    {
    ...
    // { key: "start_time", label: "Start Time", formatType: "time" },
  ];
    ...
    if (isGridDataSuccess) {
      setFilterData((prev) => {
        return {
          ...
          //     ezpassDetail?.filter_options?.available_log_types
          //   ),
          // },
          // log_status: {
          //   ...prev["log_status"],
          //   data: filterSelectGenerate(
          //     ezpassDetail?.filter_options?.available_statuses
          //   ),
          // },
        };
      });
  ...
  const searchData = (type, value) => {
    ...
    if (type.field === cabNumber) {
      queryParams.append(cabNumber, value);
    }
    if (type.field === resolutionStatus) {
      queryParams.append(resolutionStatus, value);
    }
    // if (type.field === tripDate) {
    //     queryParams.append(tripDate, value);
    // }
    ...
    triggerGetEzpassSearch(`?${queryParams?.toString()}`);
  };
  ...
  const updateFilter = (option, data = null, action = "add") => {
    ...
    console.log("🚀 ~ updateFilter ~ fieldMapping:", fieldMapping);
  ...
  useEffect(() => {
    if (ezpassSearchDetail) {
      ...
      if (filterSearchBy === startLocationgps) {
        const data = ezpassSearchDetail.items.map((item) => ({
          name: item[startLocationgps],
          id: item.id,
        }));
        handleSearchItemChange(data);
      }
  ...
  const customRender = (column, rowData) => {
    ...
    if (column.field === startLocationgps) {
      return (
        <p>
          {rowData?.["gps_start_lat"]} | {rowData?.["gps_start_lon"]}
        </p>
      );
    }
    if (column.field === tripId) {
      return (
        <p style={{ color: "#1056EF", cursor: "pointer" }}>
    ...
    if (column.field === "start_time") {
      return <p>{rowData?.start_time}</p>;
    }
    if (column.field === "end_time") {
      return <p>{rowData?.end_time}</p>;
    }
  ...
  const items = [
    ...
    {
      template: () => (
        <Link to={`/curb-trips`} className="font-semibold text-black">
          View CURB Data
        </Link>
      ),
    },
    ...
    <div className="common-layout w-100 h-100 d-flex flex-column gap-4">
      <div>
        ...
        <div className="d-flex align-items-center justify-content-between w-100">
          <div>
            <p className="topic-txt">View CURB Data</p>
