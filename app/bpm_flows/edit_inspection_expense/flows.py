...
from app.vehicles.schemas import (
    ...
    VehicleInspectionsComplianceExpenseSchema,
    VehicleStatus,
)
...
def fetch_vehicle_inspection_expenses_and_compliance(db, case_no, case_params=None):
    try:
        ...
        vehicle_details["medallion_number"] = (
            medallion.medallion_number if medallion else None
        )
        ...
        logger.error(
            "Error fetching vehicle inspection expenses and compliance: %s", str(e)
        )
        raise e
...
@step(
    step_id="218",
    name="process - Edit Vehicle Inspection Expenses and Compliance",
    operation="process",
)
def process_edit_vehicle_inspection_expenses_and_compliance(db, case_no, step_data):
    ...
    try:
        ...
        inspection_exists = vehicle_service.get_inspection(
            ...
            inspection_type=expense.sub_type,
            inspection_date=expense.issue_date,
        )
        ...
        vehicle_service.upsert_inspection(db=db, inspection_data=inspection_data)
        ...
        logger.error(
            "Error processing edit vehicle inspection expenses and compliance: %s",
            str(e),
        )
        raise e
