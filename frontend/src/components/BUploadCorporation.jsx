...
const BUploadCorporation = ({
  ...
  useEffect(() => {
    console.log("API data", apiData, file, owner_index);
    ...
    if (apiData) {
      ...
      const apiDocTypeCode = apiData.document_type;
      // Removes the suffix like "_1", "_2" to find the original option
      const baseDocTypeCode = apiDocTypeCode.replace(/_\d+$/, "");
      const selectedOption = document_type?.find(
        (item) => item?.code === baseDocTypeCode
      );
      formik.setFieldValue("documentType", selectedOption || null, false);
      formik.setFieldValue(
        "documentDate",
  ...
  const formik = useFormik({
    ...
    onSubmit: (values) => {
      ...
      if (!apiData?.document_id || isAlwaysUpload) {
        formdata.append("file", file);
      }
      // formdata.append("file", file);
      formdata.append("notes", values[variable.field_03.id]);
      formdata.append("object_type", object_type);
      const selectedDocType = values.documentType;
      let finalDocType = selectedDocType.code;

      // Check if the type needs the index appended
      if (
        (selectedDocType.code === "driving_license" ||
          selectedDocType.code === "passport") &&
        owner_index !== undefined
      ) {
        finalDocType = `${selectedDocType.code}_${owner_index + 1}`;
      }
      ...
      if (values[variable.field_02.id].code === "others") {
        // get the file name without extension
        const rawName = file?.name || document_name || "unknown";
        let nameWithoutExt =
          rawName.split(".").slice(0, -1).join(".") || rawName;
  ...
  }, [isDocSuccess, isFetching, action, onSuccess]);
  ...
  const customBase64Uploader = async (event) => {
    ...
    // }
    if (!allowedTypes.includes(file.type)) {
      // invalid type, clear selection if possible
          ...
          {file?.name && !isAlwaysUpload ? (
            ...
            <div className="attach-con">
              <FileUpload
                ...
                invalidFileTypeMessage="Only PDF, JPG, JPEG and PNG files are allowed"
                customUpload={true}
                auto
...
export default BUploadCorporation;
