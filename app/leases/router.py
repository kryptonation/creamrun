...
def view_lease(
    ...
    try:
        ...
        additional_drivers = (
            [
                lease_driver.driver.to_dict()
                for lease_driver in lease_drivers
                if lease_driver.is_additional_driver is True
                and lease_driver.is_active is True
            ]
            if lease_drivers
            else []
        )
...
def process_lease_auto_renewals(
    ...
    renewal_date: Optional[str] = Query(
        None, description="Date to process renewals for (YYYY-MM-DD). Defaults to today."
    ),
    logged_in_user: User = Depends(get_current_user),
):
    """
