...
const RenewFinancialInfo = ({ caseId, caseData, currentStepId, reload, hasAccess }) => {
    ...
    const formik = useFormik({
        ...
        onSubmit: (values) => {
            console.log(values);
            const weekDays = values?.["day_of_week"] ? values?.["day_of_week"]?.map(item => item.code) : ""

            let weekShifts;
            if (weekDays) {
                weekShifts = weekDays?.reduce((acc, day) => {
                    acc[day] = {
                        day_shift: values?.[`${day}_longer_day_shift`],
                        night_shift: values?.[`${day}_longer_day_shift`]
                    };
                    return acc;
                }, {});
            }
            ...
            const data = {
                dov: {
                    "management_recommendation": stepInfoData?.financials?.management_recommendation,
                    "med_lease": stepInfoData?.financials?.med_lease,
                    "med_tlc_maximum_amount": stepInfoData?.financials?.tlc_medallion_cap,
                    "veh_lease": stepInfoData?.financials?.veh_lease,
                    "veh_tlc_maximum_amount": stepInfoData?.financials?.tlc_max_vehicle_cap,
                    "lease_amount": stepInfoData?.financials?.lease_amount
                },
                ["long-term"]: {
                    "management_recommendation": stepInfoData?.financials?.management_recommendation,
                    "day_shift": stepInfoData?.financials?.day_shift_amount,
                    "day_tlc_maximum_amount": stepInfoData?.financials?.tlc_medallion_cap,
                    "night_shift": stepInfoData?.financials?.night_shift_amount,
                    "night_tlc_maximum_amount": stepInfoData?.financials?.tlc_max_vehicle_cap,
                    "lease_amount": stepInfoData?.financials?.lease_amount
                },
                ["medallion-only"]: {
                    "weekly_lease_rate": stepInfoData?.financials?.lease_amount,
                    "week_tlc_maximum_amount": stepInfoData?.financials?.lease_amount,
                },
                ["short-term"]: {
                    "day_shift_amount": stepInfoData?.financials?.day_shift_amount,
                    "night_shift_amount": stepInfoData?.financials?.night_shift_amount,
                }
            }
...
export default RenewFinancialInfo