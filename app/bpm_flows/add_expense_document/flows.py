...
@step(step_id="220" , name="Fetch - Vehicle Expenses other documents", operation='fetch')
def fetch_vehicle_expense_other_document(db, case_no, case_params=None):
    try:
        ...
        if not case_entity or not expense:
            expense = vehicle_service.upsert_vehicle_expenses(
                db=db , vehicle_expenses={
                    "vehicle_id": vehicle.id,
                    "category": ExpensesAndComplianceCategory.OTHER_VHICLE_DOCUMENTS.value
                }
            )
...
@step(step_id="220" , name="process - Vehicle expenses documents", operation="process")
def process_vehicle_expenses_documents(db, case_no, step_data):
    """
    Process the vehicle expenses docuements.
    """
    try:
        logger.info("Process vehicle expenses document for case_no: %s", case_no)
        ...
        if case:
            audit_trail_service.create_audit_trail(
                db=db,
                case=case,
                description=f"Vehicle Expense Document Uploaded for vehicle vin:{expense.vehicle.vin if expense.vehicle else 'N/A'}",
                meta_data={"vehicle_id":expense.vehicle_id}
            )

        return "Ok"
    except Exception as e:
        logger.error("Error processing vehicle expenses document: %s", str(e))
        raise e