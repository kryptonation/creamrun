...
from app.vehicles.schemas import VehicleRepairsMaintenanceExpenseSchema, ExpensesAndComplianceCategory , ExpensesAndComplianceSubType , VehicleStatus
...
@step(step_id="215" , name="Fetch - Vehicle Repaire Expenses", operation='fetch')
def fetch_vehicle_repire_expenses(db, case_no, case_params=None):
    try:
        ...
        if not case_entity:
            expense = vehicle_service.upsert_vehicle_expenses(
                db=db , vehicle_expenses={
                    ...
                    "category": ExpensesAndComplianceCategory.REPAIRS_AND_MAINTENANCE.value,
                    "amount": 0.0
                }
            )
        ...
        invoice = upload_service.get_documents(
            db=db , object_type="vehicle_expenses" , object_id=expense.id , document_type="repire_invoice"
        )
...
@step(step_id="215" , name="process - Vehicle Repaire Expenses", operation="process")
def process_vehicle_repaire_expenses(db, case_no, step_data):
    """
    Process the vehicle repairs expenses
    """
    try:
        logger.info("Process vehicle repaire expenses")
        ...
        if case:
            audit_trail_service.create_audit_trail(
                db=db,
                case=case,
                description=f"Vehicle Repaire expenses processed for vehicle {expense.vehicle.vin if expense.vehicle else "N/A"}",
                meta_data={"vehicle_id":expense.vehicle_id}
            )

        return "Ok"
    except Exception as e:
        logger.error("Error processing vehicle repaire expenses: %s", str(e))
        raise e