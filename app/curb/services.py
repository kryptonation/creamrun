...
from app.curb.exceptions import CurbApiError, DataMappingError, TripProcessingError
...
class CurbService:
        ...
        try:
            ...
            for trip_node in trip_nodes:
                ...
                try:
                    ...
                    payment_type_str = (
                        get_value(trip_node, "PAYMENT_TYPE")
                        or get_value(trip_node, "CC_TYPE")
                        or ""
                    )
                    payment_type_char = (
                        payment_type_str[0] if payment_type_str else None
                    )

                    if (
                        payment_type_char == "$"
                        or "cash" in (trip_node.findtext("PAYMENT_TYPE") or "").lower()
                    ):
                        payment_type = PaymentType.CASH
                    elif payment_type_char in [
                        "C",
                        "1",
                        "2",
                        "3",
                    ...
                    trip_id = trip_node.attrib.get("ROWID") or trip_node.attrib.get(
                        "RECORD ID"
                    )
                    ...
                    def parse_flexible_datetime(datetime_str: str) -> datetime:
                        ...
                        try:
                            ...
                            try:
                                ...
                                logger.warning(
                                    f"Failed to parse datetime: '{datetime_str}'"
                                )
                                raise
                    ...
                    if not cab_number:
                        ...
                        cab_number = get_value(trip_node, "CAB_NUMBER") or get_value(
                            trip_node, "CabNumber"
                        )
                    ...
                    if not cab_number and trip_node.tag == "tran":
                        ...
                        logger.warning(
                            f"Missing CABNUMBER in transaction {unique_id}. Available attributes: {list(trip_node.attrib.keys())}"
                        )
                        ...
                        logger.debug(
                            f"Found CABNUMBER: {cab_number} for transaction {unique_id}"
                        )
                    ...
                    trip_data = {
                        ...
                        "curb_driver_id": get_value(trip_node, "DRIVER")
                        or get_value(trip_node, "TRIPDRIVERID"),
                        "curb_cab_number": cab_number,
                        "start_time": start_time,
                        "end_time": end_time,
                        "fare": Decimal(get_value(trip_node, "TRIPFARE") or "0.00"),
                        "tips": Decimal(
                            get_value(trip_node, "TRIPTIPS")
                            or get_value(trip_node, "TIPS")
                            or "0.00"
                        ),
                        "tolls": Decimal(get_value(trip_node, "TRIPTOLL") or "0.00"),
                        "extras": Decimal(get_value(trip_node, "TRIPEXTRAS") or "0.00"),
                        "total_amount": Decimal(
                            get_value(trip_node, "AMOUNT")
                            or get_value(trip_node, "TOTAL_AMOUNT")
                            or "0.00"
                        ),
                        "surcharge": Decimal(get_value(trip_node, "TAX") or "0.00"),
                        "improvement_surcharge": Decimal(
                            get_value(trip_node, "IMPTAX") or "0.00"
                        ),
                        "congestion_fee": Decimal(
                            get_value(trip_node, "CongFee") or "0.00"
                        ),
                        "airport_fee": Decimal(
                            get_value(trip_node, "airportFee") or "0.00"
                        ),
                        "cbdt_fee": Decimal(get_value(trip_node, "cbdt") or "0.00"),
                        "start_long": Decimal(
                            get_value(trip_node, "GPS_START_LO")
                            or get_value(trip_node, "FromLo")
                            or None
                        ),
                        "start_lat": Decimal(
                            get_value(trip_node, "GPS_START_LA")
                            or get_value(trip_node, "FromLa")
                            or None
                        ),
                        "end_long": Decimal(
                            get_value(trip_node, "GPS_END_LO")
                            or get_value(trip_node, "ToLo")
                            or None
                        ),
                        "end_lat": Decimal(
                            get_value(trip_node, "GPS_END_LA")
                            or get_value(trip_node, "ToLa")
                            or None
                        ),
                    ...
                    logger.warning(
                        "Skipping malformed trip record: %s. Error: %s",
                        ET.tostring(trip_node, "utf-8"),
                        e,
                    )
                    continue
    ...
    def _reconcile_locally(self, trips: List[CurbTrip]) -> int:
        ...
        self.db.commit()
        logger.info(
            f"Locally marked {len(trips)} trips as RECONCILED for non-production environment."
        )
    ...
    def import_and_reconcile_data(
        ...
        try:
            ...
            if medallion_no:
                medallion_numbers = [
                    m.strip() for m in medallion_no.split(",") if m.strip()
                ]
                logger.info(
                    f"Starting CURB import for specified medallions: {medallion_numbers}"
                )
            else:
                logger.info(
                    "Starting CURB import for all active medallions in the system."
                )
                active_medallions = (
                    self.db.query(Medallion.medallion_number)
                    .filter(Medallion.medallion_status == MedallionStatus.ACTIVE)
                    .all()
                )
                medallion_numbers = [m[0] for m in active_medallions]
                if not medallion_numbers:
                    return {
                        "message": "No active medallions found in the system to import data for."
                    }
            ...
            for cab_number in medallion_numbers:
                try:
                    ...
                    trips_log_xml = self.api_service.get_trips_log10(
                        from_date_str, to_date_str, cab_number=cab_number
                    )
                    ...
                    logger.error(
                        f"Failed to fetch data for medallion {cab_number}: {e}"
                    )
                    api_errors.append({"medallion_number": cab_number, "error": str(e)})

            final_trip_list = list(all_trips_data.values())
            logger.info(
                f"Fetched a total of {len(final_trip_list)} unique records from CURB."
            )
            ...
            self.db.rollback()
            logger.error(
                "An unexpected error occurred during CURB import: %s", e, exc_info=True
            )
            raise
    ...
    def map_reconciled_trips(self) -> Dict:
        ...
        logger.info(
            f"Found {len(trips_to_map)} reconciled trips to process for mapping."
        )
        ...
        for trip in trips_to_map:
            try:
                ...
                driver = driver_service.get_drivers(
                    db=self.db, tlc_license_number=trip.curb_driver_id
                )
                if not driver:
                ...
                medallion = medallion_service.get_medallion(
                    db=self.db, medallion_number=trip.curb_cab_number
                )
                if not medallion:
                ...
                if not lease:
                    raise DataMappingError(
                        "Lease",
                        f"Driver ID {driver.driver_id} & Medallion {medallion.medallion_number}",
                    )
                ...
                logger.error(
                    f"Unexpected error mapping trip {trip.curb_trip_id}: {e}",
                    exc_info=True,
                )
                continue
        ...
        try:
            ...
            self.db.rollback()
            logger.error(
                "Database commit failed during trip mapping: %s", e, exc_info=True
            )
    ...
    def post_mapped_earnings_to_ledger(
        ...
        try:
            ...
            if not trips_to_post:
                logger.info(
                    "No new mapped credit card earnings found to post to the ledger."
                )
                return {
                    "drivers_processed": 0,
                    "trips_posted": 0,
            ...
            for trip in trips_to_post:
                if trip.driver_id is None or trip.lease_id is None:
                    logger.warning(
                        f"Skipping trip {trip.curb_trip_id} as it is missing driver_id or lease_id."
                    )
                    continue
            ...
            for (
                driver_id,
                lease_id,
            ), total_earnings in earnings_by_driver_lease.items():
                try:
                    logger.info(
                        f"Posting earnings of ${total_earnings} for driver_id {driver_id} on lease {lease_id}."
                    )
                    ...
                    for trip_id in trip_ids_to_update:
                        self.repo.update_trip_status(
                            trip_id, CurbTripStatus.POSTED_TO_LEDGER
                        )
            ...
            self.db.rollback()
            logger.error(
                "A critical error occurred during the earnings posting process: %s",
                e,
                exc_info=True,
            )
