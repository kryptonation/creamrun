            ...
            if medallion_info:
                ...
                if medallion_owner:
                    audit_trail_service.create_audit_trail(
                        db=db,
                        case=case,
                        description=f"Processed vehicle hackup for vehicle {vehicle.vin} with medallion {medallion_info.medallion_number} for owner {medallion_owner.id}",
        ...
        if case:
            ...
            if medallion_owner:
                audit_trail_service.create_audit_trail(
                    db=db,
                    case=case,
                    description=f"Processed vehicle inspection for vehicle {vehicle.vin} with medallion {medallion.medallion_number} for owner {medallion_owner.id}",
                    meta_data={"medallion_owner_id": medallion_owner.id , "medallion_id": medallion.id , "vehicle_id": vehicle.id}
                )
        return "Ok"
    except Exception as e:
            ...
            if medallion_info:
                ...
                if medallion_owner:
                    audit_trail_service.create_audit_trail(
                        db=db,
                        case=case,
                        description=f"Processed upload invoices for vehicle {vehicle.vin} with medallion {medallion_info.medallion_number} for owner {medallion_owner.id}",
                        meta_data={"medallion_owner_id": medallion_owner.id , "medallion_id": medallion_info.id , "vehicle_id": vehicle.id}
                    )
