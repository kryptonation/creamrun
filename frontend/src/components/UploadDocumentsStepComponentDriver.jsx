...
const UploadDocumentsStepComponentDriver = ({
  caseId,
  reload,
  currentStep = {},
  hasAccess,
  ...
  const docsArray = Array.isArray(currentStep?.documents)
  ...
  const transformedStep = {
    ...
    documents: docsArray.map((doc) => ({
      ...
      document_type: doc.document_type, // keep top-level type
  ...
  const customRender = (column, rowData) => {
    if (column.field === "document_name") {
      ...
      const isRequired = transformedStep?.required_documents?.includes(
  ...
  const areAllRequiredDocumentsUploaded = () => {
    if (!transformedStep?.required_documents || !transformedStep?.documents) {
      return false;
    }

    const requiredTypes = new Set(transformedStep?.required_documents);
    const uploadedRequiredTypes = new Set();

    transformedStep.documents.forEach((doc) => {
      if (
  ...
  return (
    <div>
      <DataTableComponent
        columns={columns}
        paginator={false}
        data={transformedStep?.documents || []}
...
export default UploadDocumentsStepComponentDriver;
