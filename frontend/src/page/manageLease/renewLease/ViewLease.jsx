...
import { medallionApi, useMoveCaseDetailMutation, useProcessCaseDeatilMutation } from '../../../redux/api/medallionApi.js'
...
import { DISABLEDAYNIGHTSHIFT } from '../../../utils/constants.js'
import { Checkbox } from 'primereact/checkbox';

const ViewLease = ({ caseId, caseData, currentStepId, currentStep, currentStepData, reload, hasAccess }) => {
  ...
  // const { data: currentStep, isSuccess } = useGetStepInfoQuery({ caseNo: caseId, step_no: currentStepId }, { skip: !currentStepId || !caseId });
  ...
  useEffect(() => {
    if (currentStep) {
      const stepData = currentStep?.lease_info;
      const stepDataLease = currentStep?.lease_case_details;
      ...
      // formik.setFieldValue([variable?.field_08.id], stepData?.[variable?.field_08.id] || variable?.field_08.value, true);
