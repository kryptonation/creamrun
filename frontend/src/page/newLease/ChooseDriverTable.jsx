...
const ChooseDriverTable = ({
  ...
  const triggerSearchDriverData = ({ page, limit }) => {
    ...
    // queryParams.append("driver_status", "registered");
      ...
      return (
        <div className="d-flex align-items-center gap-1">
          ...
          <InputSwitch
            ...
            onChange={(e) => handleShiftChange(rowData?.driver_lookup_id, e.value)}
          />
          N
        </div>
      );
  ...
  const handleSelectionChange = (e) => {
    ...
    if (selectionMode === "single") {
      ...
      const deselected = selectedDrivers.filter(
        (driver) =>
          !newSelection.some((sel) => sel.driver_lookup_id === driver.driver_lookup_id)
      );
      ...
      const filteredSelection = previousSelectedDriver
        ? newSelection.filter(
          (driver) => driver.driver_lookup_id !== previousSelectedDriver.driver_lookup_id
        )
        : newSelection;
...
export default ChooseDriverTable;
